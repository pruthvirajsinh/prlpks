...
//PRC Start
type LcElements struct {
	remoteElements *ZSet
	localElements  *ZSet
}

func (msg *LcElements) String() string {
	return fmt.Sprintf("%v", msg.MsgType())
}

func (msg *LcElements) MsgType() MsgType {
	return MsgTypeLcElements
}

func (msg *LcElements) marshal(w io.Writer) (err error) {
	err = WriteZSet(w, msg.remoteElements)
	if err != nil {
		return
	}
	err = WriteZSet(w, msg.localElements)
	return
}

func (msg *LcElements) unmarshal(r io.Reader) (err error) {
	msg.remoteElements, err = ReadZSet(r)
	if err != nil {
		return
	}
	msg.localElements, err = ReadZSet(r)
	return
}
